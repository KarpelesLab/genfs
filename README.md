...
| 7z / rar / arc / lha / sit | 🚧 | — | — | detected by `info`; reader not implemented yet (returns a clear `Unsupported`) — pure-Rust decoders land behind per-format Cargo features |
...
`repack`, which rebuilds. `cab` (Store/MSZIP/LZX/Quantum) and `lzx` (Amiga
LZX) are read-only readers via `compcol`, behind the `cab` / `amiga-lzx`
features. `7z`, `rar`, `arc`, `lha`, and `sit` are recognised by `info`
today but their readers are scaffolds that return a clear `Unsupported`;
pure-Rust decoders will land behind per-format Cargo features. (`rar` and `sit` are read-only-at-best — their creation is
proprietary.)
